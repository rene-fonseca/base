...
//     return;
//   }
//   _throw IEEE1394Exception("Invalid request.", this);
// }
...
unsigned int IEEE1394::getVendorId(unsigned short node)
{
  ...
  _throw IEEE1394Exception("Invalid configuration ROM.", this);
}
  ...
  for (unsigned int entry = 1; entry <= numberOfEntries; ++entry) {
    ...
    if (keyValue == IEEE1394::KEY_TEXTUAL) {
      ...
      if (keyType == IEEE1394::KEY_TYPE_LEAF) {
        ...
        if (leafSize > 2) {
          PrimitiveStackArray<Quadlet> leaf(leafSize); // must be big endian
...
String IEEE1394::getKeywords(unsigned short node) {
  ...
  for (unsigned int entry = 1; entry <= numberOfEntries; ++entry) {
    ...
    if ((keyType == IEEE1394::KEY_TYPE_LEAF) &&
      ...
      uint32 leafOffset = entryOffset + (quadlet & 0x00ffffff) * sizeof(Quadlet);
      const unsigned int leafSize = getQuadlet(node, leafOffset) >> 16;
      PrimitiveStackArray<Quadlet> leafWords(leafSize); // must be big endian
      for (unsigned int i = 0; i < leafSize; ++i) {
        leafOffset += sizeof(Quadlet);
...
void IEEE1394::loadTopologyMap()
{
  ...
  // get topology map
  PrimitiveStackArray<uint32> ids(selfIds);
...
void IEEE1394::loadSpeedMap() {
  ...
  if ((crc >> 16) != ((64 * 62 + 62 + 3)/4 + 1)) {
    _throw IEEE1394Exception("Invalid speed map.", this);
  }
...
uint64 IEEE1394::IsochronousReadChannelImpl::getSubchannels(
  ...
  _throw IEEE1394Exception("Channel is closed.", this);
}
