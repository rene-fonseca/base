  ...
  : path(_path), mode(0), links(0)
{
  ...
  while ((folder == INVALID_HANDLE_VALUE) && (++linkLevel <= maximumLinkLevel)) {
    ...
    folder = ::CreateFileW(ToWCharString(substPath), // file name
                         0 | READ_CONTROL, // access mode
                         FILE_SHARE_READ | FILE_SHARE_WRITE, // share mode
                         0, // security descriptor
                         OPEN_EXISTING, // how to create
  ...
  if (explicitOwner) {
    ...
    if (ownerAccessMask & (FILE_ADD_FILE|FILE_ADD_SUBDIRECTORY|FILE_WRITE_ATTRIBUTES|FILE_WRITE_EA|FILE_DELETE_CHILD|WRITE_DAC|WIN32_DELETE|WRITE_OWNER)) {
  ...
  if (explicitGroup) {
    ...
    if (groupAccessMask & (FILE_ADD_FILE|FILE_ADD_SUBDIRECTORY|FILE_WRITE_ATTRIBUTES|FILE_WRITE_EA|FILE_DELETE_CHILD|WRITE_DAC|WIN32_DELETE|WRITE_OWNER)) {
  ...
  if (explicitEveryone) {
    ...
    if (everyoneAccessMask & (FILE_ADD_FILE|FILE_ADD_SUBDIRECTORY|FILE_WRITE_ATTRIBUTES|FILE_WRITE_EA|FILE_DELETE_CHILD|WRITE_DAC|WIN32_DELETE|WRITE_OWNER)) {
