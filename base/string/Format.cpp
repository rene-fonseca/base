...
String String::operator()() const
{
  ...
  return Format::Subst(getSpan(), spans, getArraySize(spans)).format();
}
#endif
...
{
  return Subst(text.getSpan(), nullptr, 0).format();
}

String Format::subst(const String& text, const String& a)
{
  ConstSpan<char> spans[] = {a.getSpan()};
  return Subst(text.getSpan(), spans, getArraySize(spans)).format();
}
...
String Format::subst(const String& text, const AnyValue& a, const AnyValue& b, const AnyValue& c, const AnyValue& d,
  const AnyValue& e, const AnyValue& f, const AnyValue& g, const AnyValue& h)
{
  return subst(text, a.getString(), b.getString(), c.getString(), d.getString(), e.getString(), f.getString(),
    g.getString(), h.getString());
}

String Format::subst(const String& text, const AnyValue& a, const AnyValue& b, const AnyValue& c, const AnyValue& d,
  const AnyValue& e, const AnyValue& f, const AnyValue& g, const AnyValue& h, const AnyValue& i, const AnyValue& j, const AnyValue& k, const AnyValue& l)
{
  return subst(text, a.getString(), b.getString(), c.getString(), d.getString(), e.getString(), f.getString(),
    g.getString(), h.getString(), h.getString(), i.getString(), j.getString(), k.getString());
}

String Format::subst(const String& text, const AnyValue& a, const AnyValue& b, const AnyValue& c, const AnyValue& d,
  const AnyValue& e, const AnyValue& f, const AnyValue& g, const AnyValue& h, const AnyValue& i)
{
  return subst(text, a.getString(), b.getString(), c.getString(), d.getString(), e.getString(), f.getString(),
    g.getString(), h.getString(), h.getString(), i.getString());
}

String Format::subst(const String& text, const AnyValue& a, const AnyValue& b, const AnyValue& c, const AnyValue& d,
  const AnyValue& e, const AnyValue& f, const AnyValue& g, const AnyValue& h, const AnyValue& i, const AnyValue& j)
{
  return subst(text, a.getString(), b.getString(), c.getString(), d.getString(), e.getString(), f.getString(),
    g.getString(), h.getString(), h.getString(), i.getString(), j.getString());
}
...
String Format::subst(const String& text, const std::initializer_list<const char*>& list)
{
  ...
  return Subst(text.getSpan(), spans, j).format();
}
